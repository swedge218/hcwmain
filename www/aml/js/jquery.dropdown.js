...
;(function( $, window, document, undefined ) {
	...
	dropdown.prototype = {
		...
		resize: function( menu, noAnimation ) {
			...
			$list.css({ height: resize.collision.height.list - 5 });
		...
		_bindEvents: function() {
			...
			elem.dropdown.on( 'click', '.' + cls.core.menuItem, function(e) {
				...
				self.select( item );
                                ...
                                /**
                                * LEKE added this. Efforts to have it in another separate method proved complex
                                * and pressed for time
                                * Set up the selected value into the watcher text box for the selected element
                                * =============================================================== */
                                   children  = $(this).children();
                                   child = children.find('.dropdown-text');
                                   childText = children.find('.dropdown-text').text();
                                   selectID = $(this).parents(".dropdown").siblings("select").prop('id');
                                   options = document.getElementById(selectID).options;
                                   console.log(options.length);
                                   for(var i=0; i<options.length; i++){
                                       value = options[i].getAttribute("value");
                                       optionText = options[i].innerHTML;
                                       if(childText == optionText){
                                           var hiddenField = document.querySelector('#'+selectID + ' ~ .watcher');
